# The Canonical Distribution of Kubernetes - designed for Canonical Openstack Private Cloud

![](https://img.shields.io/badge/kubernetes-1.12-brightgreen.svg) ![](https://img.shields.io/badge/juju-2.0+-brightgreen.svg)
...
### Configuring the Openstack Integrator Charm

This bundle file uses the [Openstack Integrator Charm]
(https://github.com/juju-solutions/charm-openstack-integrator) which requires a
project (tennant) to be created within Openstack along with some networks to be created
for the subnet of Kubernetes and the load-balancers. 

Within the included bundle, the credentials including the network IDs have been placed
directly into the options section of the Openstack integrator charm. The credentials
and ID(s) for the various networks should be derived using the openstack CLI tool 
and replaced within the bundle file before deployment. 

It is also possible to store the credentials inside a Juju credential, 
which can be configured using the Juju manual. Once configured, you can allow 
juju to 'trust' the integrator charm using the following command: 

```
juju trust openstack-integrator
```

The integrator charm allows Kubernetes to interact with its underlying infrastructure 
to provision load-balancers and storage which can be consumed directly by Kubernetes. 
After deployment, you must create a storage class for Cinder. As long as the floating-ip
network ID and the kubernetes subnet ID are correct, load-balancer creation should be
automatic when a new load-balancer service is defined. 
