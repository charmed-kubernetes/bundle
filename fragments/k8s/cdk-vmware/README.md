# The Canonical Distribution of Kubernetes - designed for VMware vSphere
...
### Configuring the VMware vSphere Integrator Charm

This bundle file uses the [VMware vSphere Integrator Charm]
(https://github.com/juju-solutions/charm-vsphere-integrator) which requires custom 
...
This will allow Juju to grant the vSphere integrator charm the credentials, instead of
...
to provision storage consumed directly by Kubernetes. After deployment, you must create 
a storage class for VMware and the account defined in the credentials file must be 
able to provision and attach storage to VM(s).
