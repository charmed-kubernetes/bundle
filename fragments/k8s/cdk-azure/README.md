# The Canonical Distribution of Kubernetes - designed for Microsoft Azure Public Cloud
...
### Configuring the Azure Integrator Charm

This bundle file uses the [Azure Integrator Charm]
(https://github.com/juju-solutions/charm-azure-integrator) which requires custom 
credentials to be created within Juju. Instructions for this are provided in the Juju
manual. 

After deploying with Juju, make sure you run the command: 
```
juju trust azure-integrator
```

This will allow Juju to grant the Azure integrator charm the credentials, instead of
storing them directly in the plain-text bundle file.
...
After deployment, you must create a storage class for azure. Load-balancer creation
should be automatic, assuming your azure account has the correct permissions to both
create and manipulate load-balancers or storage, if required.
